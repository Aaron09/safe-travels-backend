...
def county_rating(request, county_id):
    query = "select rating from (select county_id, avg(rating) as rating from app_county join app_review on app_review.county_id = app_county.id group by county_id) as V where county_id={0}".format(county_id)
